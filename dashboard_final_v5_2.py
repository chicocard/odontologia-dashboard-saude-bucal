...
st.set_page_config(
    page_title="OdontoWorkforce Brasil — V5.2",
    page_icon="🦷",
    layout="wide",
...
def normalize_municipio_code_py(value: Any) -> str:
    """
    Normaliza código municipal para 6 dígitos, compatível com DATASUS/regiões de saúde.

    Motivo:
    - algumas bases RAIS trazem código municipal com 6 dígitos;
    - outras podem vir com 7 dígitos IBGE completo ou com sufixo decimal convertido em texto;
    - usar sempre os 6 últimos dígitos distorce códigos 7-dígitos, por exemplo 3106200 -> 106200.
    """
    if pd.isna(value):
        return ""
...
def mun_key_expr(col: str) -> str:
    ...
    CASE
        ...
        WHEN LENGTH({digits}) >= 6 THEN RIGHT({digits}, 6)
        ELSE LPAD({digits}, 6, '0')
    END
    """
...
st.markdown(
    ...
    <div class="ow-hero">
        ...
        <div class="ow-hero-subtitle">
            Painel V5.2 — força de trabalho odontológica, território, especialidades, vínculos e taxas per capita.
        </div>
...
def build_population_panel(terr: pd.DataFrame, counts: pd.DataFrame, years_selected: list[int]) -> pd.DataFrame:
    ...
    counts2 = counts.copy()
    counts2["cod_municipio"] = counts2["cod_municipio"].apply(normalize_municipio_code_py)
...
with tabs[7]:
    ...
    download_buttons(export_tables["serie_temporal"], "exportacao_geral_v5_2_2", export_tables)
...
st.markdown(
    '<div class="ow-footer">OdontoWorkforce Brasil V5.2 — painel experimental para análise de força de trabalho odontológica com RAIS, regiões de saúde e populações IBGE/TCU.</div>',
    unsafe_allow_html=True,
)
