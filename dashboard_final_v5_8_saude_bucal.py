...
st.set_page_config(
    page_title="OdontoWorkforce Brasil — V5.8 Saúde Bucal",
    page_icon="🦷",
    layout="wide",
...
st.markdown(
    ...
    <div class="ow-hero">
        ...
        <div class="ow-hero-subtitle">
            Painel V5.8 Saúde Bucal — composição ocupacional, densidades, território, vínculos e perfil sociodemográfico.
        </div>
...
with tabs[2]:
    st.markdown('<div class="ow-section-title">Território: densidades, desigualdade e ranking</div>', unsafe_allow_html=True)

    st.markdown(
        '<div class="ow-note">'
        'A leitura territorial foi redesenhada para evitar boxplots confusos. '
        'Agora a aba combina: distribuição resumida por boxplot, ranking horizontal, dispersão população × densidade '
        'e tabela-síntese. O boxplot pode ser ajustado por nível territorial, categoria ocupacional e escala.'
        '</div>',
        unsafe_allow_html=True,
    )
    ...
    if municipal_panel.empty or boletim_panel.empty:
        ...
        csel1, csel2, csel3, csel4 = st.columns([1.1, 1.1, 1, 1])
        ...
        with csel1:
            nivel_territorial = st.selectbox(
                ...
                index=1,
                help="Região de saúde e macrorregião costumam produzir leituras mais úteis para planejamento regional.",
            )
        ...
        def build_territory_summary(level: str, categoria: str) -> pd.DataFrame:
            if level == "UF":
                keys = ["ano", "sg_uf"]
                label_name = "territorio"
                pop = (
                    municipal_panel.groupby(keys, as_index=False)
                    .agg(populacao=("populacao_usada", "sum"))
                )
                pop[label_name] = pop["sg_uf"]
                ...
                pop[label_name] = pop["sg_uf"].astype(str) + " — " + pop["macrorregiao_de_saude"].astype(str)
                ...
                pop[label_name] = pop["sg_uf"].astype(str) + " — " + pop["regiao_de_saude"].astype(str)
            else:
                keys = ["ano", "cod_municipio", "municipio_label", "sg_uf"]
                label_name = "territorio"
                pop = (
                    municipal_panel.groupby(keys, as_index=False)
                    .agg(populacao=("populacao_usada", "max"))
                )
                pop[label_name] = pop["municipio_label"].astype(str)

            if categoria == "Total Saúde Bucal":
                work = (
                    boletim_panel.groupby(keys, as_index=False)
                    .agg(vinculos=("vinculos", "sum"))
                )
            else:
                work = (
            ...
            out = out.rename(columns={label_name: "territorio"})
            return out
        ...
        if terr_df.empty:
            ...
            # Indicadores sintéticos do ano selecionado
            ...
            st.markdown("### Distribuição das densidades por ano")
            ...
            with b1:
                mostrar_pontos = st.toggle(
                    ...
                    value=False,
                    help="Desligado por padrão para reduzir poluição visual. Ligue para identificar a dispersão dos territórios.",
                )
            ...
            with b3:
                ordenar_ano = st.toggle("Ano como categoria", value=True)
            ...
            st.markdown(
                '<div class="ow-small">'
                'Leitura sugerida: use o boxplot para comparar mediana, intervalo interquartil e assimetria entre anos. '
                'Use o ranking abaixo para identificar quais territórios explicam os extremos.'
                '</div>',
                unsafe_allow_html=True,
            )
            ...
            st.markdown("### População e densidade")

            scatter = ref.copy()
            scatter["vinculos_plot"] = scatter["vinculos"].clip(lower=1)
            ...
            fig = px.scatter(
                ...
                title=(
                    f"Relação entre população e densidade — {CATEGORIA_SB_NOMES.get(categoria_territorio, categoria_territorio)}, "
                    f"{ano_territorio}"
                ),
                labels={
                    "populacao": "População",
                    "taxa_10000": "Vínculos por 10 mil habitantes",
                    "vinculos_plot": "Vínculos",
                },
            )
            ...
            download_buttons(
                table_out,
                "territorio_densidades_saude_bucal",
                {
...
with tabs[7]:
    ...
    download_buttons(export_tables["serie_temporal"], "exportacao_geral_v5_8_saude_bucal_2", export_tables)
...
st.markdown(
    '<div class="ow-footer">OdontoWorkforce Brasil V5.8 Saúde Bucal — painel experimental para análise de força de trabalho odontológica com RAIS, regiões de saúde e populações IBGE/TCU.</div>',
    unsafe_allow_html=True,
)
