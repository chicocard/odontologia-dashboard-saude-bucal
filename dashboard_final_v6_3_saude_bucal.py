...
st.set_page_config(
    page_title="OdontoWorkforce Brasil — V6.3 Saúde Bucal",
    page_icon="🦷",
    layout="wide",
...
st.markdown(
    ...
    <div class="ow-hero">
        ...
        <div class="ow-hero-subtitle">
            Painel V6.3 Saúde Bucal — composição ocupacional, densidades, território, vínculos e perfil sociodemográfico.
        </div>
    ...
    if municipal_panel.empty or boletim_panel.empty:
        ...
        if terr_df.empty:
            ...
            if geojson_obj is None:
                st.warning("GeoJSON não encontrado. Confira se existem assets/br_ufs.geojson e assets/br_municipios.geojson na pasta do projeto. A V6.2 também procura recursivamente em subpastas.")
            else:
                feature_ids = {
                ...
                if len(matched_values) == 0:
                    st.error(
                        ...
                        "casou com o GeoJSON. Verifique se a base cartográfica é compatível com códigos IBGE de 6 dígitos "
                        "para municípios e siglas UF para estados."
                    )
                ...
                fig.update_traces(marker_line_width=0.15, marker_line_color="rgba(30,41,59,0.35)")
                fig.update_geos(
                    fitbounds="locations",
                    visible=False,
                    showcountries=False,
                ...
                fig.update_layout(
                    ...
                    coloraxis_colorbar=dict(thickness=14, len=0.72),
                )
                plot(fig)
...
with tabs[7]:
    ...
    download_buttons(export_tables["serie_temporal"], "exportacao_geral_v6_3_saude_bucal_2", export_tables)
...
st.markdown(
    '<div class="ow-footer">OdontoWorkforce Brasil V6.3 Saúde Bucal — painel experimental para análise de força de trabalho odontológica com RAIS, regiões de saúde e populações IBGE/TCU.</div>',
    unsafe_allow_html=True,
)
