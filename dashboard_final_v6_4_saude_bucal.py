...
st.set_page_config(
    page_title="OdontoWorkforce Brasil — V6.4 Saúde Bucal",
    page_icon="🦷",
    layout="wide",
...
def make_static_choropleth(
    ...
    *,
    ...
    """
    Mapa coroplético estático e robusto para Streamlit.
    Substitui o choropleth interativo quando o Plotly lida mal com geometrias do TabWin.
    """
    ...
    value_by_id = data_map.set_index(loc_col)[value_col].to_dict()
    ...
    for feat in geojson_obj.get("features", []):
        ...
        if match_id not in value_by_id:
            continue

        value = value_by_id.get(match_id)
        if value is None or pd.isna(value):
            continue

        geom = feat.get("geometry", {})
        for ring in polygon_rings_from_geometry(geom):
            ring = simplify_ring_for_plot(ring)
            if len(ring) < 3:
                continue
            ...
            colors.append(cmap(norm(float(value))))
        ...
        if show_labels:
            ...
            if lon is not None and lat is not None:
                label = props.get(label_col) if label_col else match_id
                label_points.append((float(lon), float(lat), str(label)))

    if not patches:
        return None

    fig, ax = plt.subplots(figsize=(12.8, 9.2), dpi=135)
    ...
    sm.set_array([])
    cbar = fig.colorbar(sm, ax=ax, fraction=0.026, pad=0.018)
    cbar.ax.set_ylabel(legend_label, rotation=90, labelpad=13, fontsize=10)
    cbar.ax.tick_params(labelsize=9)

    if show_labels and label_points:
        for lon, lat, label in label_points:
            ax.text(lon, lat, label, fontsize=7, ha="center", va="center", color="#111827")
...
st.markdown(
    ...
    <div class="ow-hero">
        ...
        <div class="ow-hero-subtitle">
            Painel V6.4 Saúde Bucal — composição ocupacional, densidades, território, vínculos e perfil sociodemográfico.
        </div>
        ...
        if terr_df.empty:
            ...
            if geojson_obj is None:
                ...
                if len(matched_values) == 0:
                    ...
                    show_map_labels = st.toggle(
                        "Mostrar rótulos no mapa",
                        value=False,
                        help="Use apenas para mapas em UF ou recortes pequenos; em municípios nacionais pode poluir a visualização.",
                    )

                    fig_map = make_static_choropleth(
                        geojson_obj,
                        map_df,
                        loc_col=loc_col,
                        ...
                        show_labels=show_map_labels and mapa_nivel == "UF",
                        label_col="sg_uf" if mapa_nivel == "UF" else None,
                    )
...
with tabs[7]:
    ...
    download_buttons(export_tables["serie_temporal"], "exportacao_geral_v6_4_saude_bucal_2", export_tables)
...
st.markdown(
    '<div class="ow-footer">OdontoWorkforce Brasil V6.4 Saúde Bucal — painel experimental para análise de força de trabalho odontológica com RAIS, regiões de saúde e populações IBGE/TCU.</div>',
    unsafe_allow_html=True,
)
